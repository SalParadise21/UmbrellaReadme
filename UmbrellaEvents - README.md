...
- ✅ Event management (start, end, delete, edit, view) with autocomplete selection
...
- ✅ **Edit events** - Update event details after creation (`/edit_event`)
- ✅ **REST API** - FastAPI-based REST API for external integrations (Java websites, etc.)
- ✅ **OAuth2 Integration** - Discord OAuth2 helpers for website authentication and role syncing
- ✅ **Role Management API** - Assign, remove, and modify Discord roles via API
- ✅ **Java Integration** - Java helper classes for OAuth2 and API client usage
...
3. Create a `.env` file in the project root (see `env.example` for all available options):
```
...
### `/edit_event`
Edit an existing event's details. Only the event creator or administrators can edit events.

**Parameters:**
- `event`: Select from a dropdown list of all non-ended events
- `name`: New name for the event (optional, leave empty to keep current)
- `description`: New description (optional, leave empty to keep current)
- `start_time`: New start time in format `YYYY-MM-DD HH:MM AM/PM` (optional)
- `end_time`: New end time in format `YYYY-MM-DD HH:MM AM/PM` (optional)
- `timezone`: Your timezone (required if changing times)
- `required_attendees`: New number of required attendees (optional)
- `voice_channel`: New voice or stage channel for the event (optional)

**What it does:**
- Updates the event in the database
- Updates the event embed message
- Updates the Discord scheduled event (if one exists)
- Updates the start notification message (if event has started)
- Shows a summary of all changes made

**Permissions:** Only the event creator or users with Administrator permissions can edit events.

**Note:** You cannot edit events that have already ended.

**Example:**
```
/edit_event event:"Operation Thunder" name:"Operation Thunder 2.0" required_attendees:25
```
...
- **Enable event start channel** - Enable posting event start notifications to a specific channel
- **Disable event start channel** - Disable start channel (notifications post in event channel)
- **Set event start channel** - Configure which channel event start notifications should be posted to
- **Enable event attendance channel** - Enable posting attendance updates to a specific channel
- **Disable event attendance channel** - Disable attendance channel
- **Set event attendance channel** - Configure which channel attendance updates should be posted to
- **Set default voice channel** - Set the default voice/stage channel for Discord scheduled events

**What it does:**
- **Event Output Channel**: When enabled, all new events created with `/create_event` will be posted to the configured channel instead of the channel where the command was used
- **Event Start Channel**: When enabled, event start notifications will be posted to the configured channel
- **Event Attendance Channel**: When enabled, attendance-related updates will be posted to the configured channel
- **Default Voice Channel**: Sets the default voice or stage channel used when creating Discord scheduled events
- The bot will verify it has permissions in the specified channels before allowing the settings

**Example:**
```
/settings action:"Set event output channel" channel:#events-channel
/settings action:"Enable event output to channel"
/settings action:"Set default voice channel" voice_channel:#General Voice
...
│   └── war_end.py              # War end command (admin)
├── helpers/
│   ├── api_helpers.py          # API helper functions (role management)
│   ├── config.py              # Configuration and constants
│   ├── database.py             # Database operations
│   ├── discord_event_sync.py   # Discord scheduled event sync handlers
│   ├── embeds.py               # Embed creation functions
│   ├── models.py               # Data models
│   ├── oauth2_example.py      # OAuth2 example code
│   ├── reactions.py            # Reaction handling logic (raw reaction events)
│   ├── startup.py              # Startup functions (reaction restoration)
│   └── sync_role.py            # Discord OAuth2 and role syncing helpers
├── api/
│   ├── __init__.py
│   ├── auth.py                 # API authentication (API key verification)
│   ├── bot_instance.py         # Bot instance management for API
│   ├── routes.py               # API route definitions
│   ├── server.py               # FastAPI server setup
│   └── README.md               # API documentation
├── java/
│   ├── ApiClient.java          # Java REST API client
│   ├── DiscordOAuth2Helper.java # Java Discord OAuth2 helper
│   ├── API_JAVA_SETUP.md       # Java API setup guide
│   ├── OAUTH2_JAVA_SETUP.md    # Java OAuth2 setup guide
│   ├── pom.xml.example         # Example Maven configuration
│   └── README.md               # Java integration overview
├── requirements.txt            # Python dependencies
├── env.example                 # Example environment variables file
├── OAUTH2_SETUP.md            # Discord OAuth2 setup guide
└── README.md                   # This file
```
...
The bot includes a FastAPI-based REST API that allows external services (like Java websites) to interact with the bot's database and manage Discord roles.
...
```env
# API Configuration
API_KEY=your-secure-api-key-here  # Generate a secure random string
API_HOST=0.0.0.0                  # IP address to bind to
API_PORT=8080                      # Port number
API_CORS_ORIGINS=http://localhost:3000,https://yourdomain.com  # Allowed origins
```
...
3. **The API server starts automatically** when the bot starts and is available at:
   - **API Base URL**: `http://localhost:8080` (or your configured host/port)
   - **API Documentation**: `http://localhost:8080/docs` (Interactive Swagger UI)
   - **Health Check**: `http://localhost:8080/health`

### API Endpoints

All endpoints (except `/` and `/health`) require authentication via the `X-API-Key` header:
```
...
- **GET `/api/v1/events`** - Get all events for a guild
  - Query parameters: `guild_id` (required), `include_ended` (optional), `war_number` (optional)
  
- **GET `/api/v1/events/{event_id}`** - Get a specific event by ID

#### Role Management Endpoints

- **POST `/api/v1/roles/assign`** - Assign a role to a user
- **POST `/api/v1/roles/remove`** - Remove a role from a user
- **PUT `/api/v1/roles/modify`** - Modify a user's roles (add and/or remove multiple roles at once)

For detailed API documentation, see `api/README.md` or visit `/docs` when the API server is running.

## OAuth2 Integration

The bot includes Discord OAuth2 helpers for website authentication and role syncing. This allows you to:
- Authenticate users via Discord OAuth2
- Sync Discord roles to your website
- Implement role-based access control

### Setup

1. **Configure OAuth2 in Discord Developer Portal:**
   - Go to your Discord application → OAuth2
   - Add redirect URIs for your website
   - Select scopes: `identify`, `guilds`, `guilds.members.read`

2. **Add OAuth2 configuration to `.env`:**
```env
...
3. **Use the helpers in your website:**
```python
from helpers.sync_role import exchange_code_for_token, sync_user_roles

# Exchange OAuth2 code for token
token_data = await exchange_code_for_token(code)

# Sync user roles
sync_result = await sync_user_roles(access_token, guild_id, bot_token)
```

For complete OAuth2 setup instructions, see `OAUTH2_SETUP.md`.

## Java Integration

The bot includes Java helper classes for integrating with the REST API and OAuth2:

- **`java/ApiClient.java`** - Java client for the REST API
- **`java/DiscordOAuth2Helper.java`** - Java helper for Discord OAuth2 authentication
- **`java/README.md`** - Java integration documentation
- **`java/API_JAVA_SETUP.md`** - API client setup guide
- **`java/OAUTH2_JAVA_SETUP.md`** - OAuth2 setup guide for Java applications

See the `java/` directory for complete Java integration documentation.
...
- **Discord Scheduled Events**: The default voice channel for Discord scheduled events can be configured via `/settings`. If not configured, events use the channel specified in the event creation.
...
- **Command Syncing**: For faster command syncing during development, you can set `TEST_GUILD_ID` in your `.env` file. Commands will sync to that guild immediately instead of globally (which can take up to 1 hour).

- **Windows Support**: The bot includes Windows console encoding fixes to properly display Unicode characters in the terminal.

- **API Security**: If `API_KEY` is not set in `.env`, the API will be unsecured (development only). Always set a secure API key in production!
...
**API not working:**
- Ensure `API_KEY` is set in your `.env` file (or API will be unsecured)
- Check that the API server started successfully (you should see a message when the bot starts)
- Verify the API port (default: 8080) is not already in use
- Check CORS settings if accessing from a website
- Visit `http://localhost:8080/docs` to view interactive API documentation

**OAuth2 not working:**
- Verify `DISCORD_CLIENT_ID` and `DISCORD_CLIENT_SECRET` are set correctly
- Ensure redirect URI in `.env` exactly matches one in Discord Developer Portal
- Check that required OAuth2 scopes are selected (`identify`, `guilds`, `guilds.members.read`)
- See `OAUTH2_SETUP.md` for complete troubleshooting guide
